...
const showTicket = (ticketID) => {
  ...
  const resolve = document.getElementById("resolve_ticket");

  $.post("/employee/activetickets", { id: ticketID, type: "request" }, (data) => {
...
const resolve_ticket = (ticketID) => {
  ...
  desc = document.getElementById('desc_msg').value
  $.post("/employee/activetickets", { id: ticketID, type: "resolve", desc: desc }, (data) => {
    if (data) {
      $("#text").fadeIn();
      $("#loader_button").fadeOut();
